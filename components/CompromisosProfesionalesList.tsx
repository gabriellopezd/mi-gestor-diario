...
export default function CompromisosProfesionalesList({ categoria }: Props) {
  const [tareas, setTareas] = useState<Tarea[]>([]);
  const [nuevaTarea, setNuevaTarea] = useState("");

  useEffect(() => {
    if (!categoria) return;
    const unsuscribir = escucharTareas(categoria, setTareas);
    return () => unsuscribir();
  }, [categoria]);

  const handleAgregar = async () => {
    if (!nuevaTarea.trim() || !categoria.trim()) return;
    await crearTarea(nuevaTarea, categoria);
    setNuevaTarea("");
  };
    ...
    <div className="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-600">
      ...
      <div className="flex gap-3 mb-6">
        <input
          ...
          placeholder="Escribe un compromiso..."
          value={nuevaTarea}
